...
from .models import Equipo, FixturePartido, Torneo
...
def obtener_equipos_para_fixture(torneo: Torneo) -> List[Equipo]:
    """Obtiene los equipos participantes ordenados de forma estable."""

    return list(
        Equipo.objects.filter(categoria__liga=torneo.liga)
        .select_related("club", "categoria")
        .order_by("club__nombre", "pk")
    )


def _emparejar(arreglo: List[Optional[Equipo]]) -> Iterable[tuple[Optional[Equipo], Optional[Equipo]]]:
    n = len(arreglo)
    for i in range(n // 2):
        yield arreglo[i], arreglo[n - 1 - i]


def _rotar(arreglo: List[Optional[Equipo]]) -> List[Optional[Equipo]]:
    if len(arreglo) <= 2:
        return arreglo[:]
    fijo = arreglo[0]
    resto = arreglo[1:]
...
def crear_fixture_para_torneo(torneo: Torneo) -> List[FixturePartido]:
    ...
    if len(equipos) < 2:
        raise FixtureGenerationError("Se requieren al menos 2 equipos para generar el fixture.")

    if FixturePartido.objects.filter(torneo=torneo).exists():
        raise FixtureAlreadyExists("El fixture ya fue generado previamente.")

    usar_bye = len(equipos) % 2 == 1
    arreglo: List[Optional[Equipo]] = equipos[:]
    if usar_bye:
        arreglo.append(None)
