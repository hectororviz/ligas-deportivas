...
class FixtureGenerationTests(TestCase):
    ...
    def crear_equipo(self, nombre: str) -> Equipo:
        club = Club.objects.create(nombre=nombre)
        return Equipo.objects.create(club=club, categoria=self.categoria)

    def test_even_number_of_teams_generates_double_round_robin(self):
        equipos = [self.crear_equipo(f"Club {i}") for i in range(4)]
        ...
        for equipo_a, equipo_b in combinations(equipos, 2):
            partido_ida = ronda1.filter(local=equipo_a, visitante=equipo_b).first()
            if partido_ida is None:
                partido_ida = ronda1.get(local=equipo_b, visitante=equipo_a)
        ...
        for equipo in equipos:
            secuencia = [
                "L" if p.local_id == equipo.id else "V"
                for p in ronda1.order_by("fecha_nro")
                if equipo.id in {p.local_id, p.visitante_id}
            ]
    ...
    def test_odd_number_of_teams_includes_bye_each_round(self):
        equipos = [self.crear_equipo(f"Club {i}") for i in range(5)]
        ...
        for equipo in equipos:
            jugadas = set(
                ronda1.filter(local=equipo).values_list("fecha_nro", flat=True)
            ) | set(ronda1.filter(visitante=equipo).values_list("fecha_nro", flat=True))
            self.assertEqual(len(jugadas), 4)
            self.assertEqual(len(fechas - jugadas), 1)

    def test_idempotent_generation(self):
        [self.crear_equipo(f"Club {i}") for i in range(4)]
        crear_fixture_para_torneo(self.torneo)
